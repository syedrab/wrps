...
const sidebarLinks = [
  ...
  { href: "/admin/checklist", label: "PRD Checklist", icon: ClipboardList },
];
...
function SidebarContent({ pathname }: { pathname: string }) {
  return (
    <div className="flex h-full flex-col" style={{ backgroundColor: currentAgency.primaryColor }}>
      ...
      {/* Nav */}
      <nav className="flex-1 px-3 py-1 space-y-0.5" aria-label="Admin navigation">
      ...
      <div className="border-t border-white/10 px-4 py-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-white truncate">Sgt. Kim Marshall</p>
            <p className="text-xs text-white/50">Agency Admin</p>
          </div>
        </div>
      </div>
    </div>
